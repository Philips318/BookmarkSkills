...
| 3 | **Air Calibration** | Calibrate detector response | Accurate HU values 所必需 |
...
| 5 | **Exam Card Selection** | 按 anatomy + age group 选择 protocol | Smart Protocoling: AI-suggested cards based on order/history |
| 6 | **Surview** | Scout scan for planning；在 surview image 上 draw scan boxes | Low-dose；NOT diagnostic quality |
...
| 类别 | Default Result Types |
|----------|---------------------|
| 通用 | Conventional + MonoE |
...
- **Exam Card Manager** = protocols 的 CRUD interface
...
- **Age Group Matching** = 为 children < 70 kg auto-select pediatric card
- **Spectral Results** = 通过 Exam Card Manager 添加（Add Spectral Result button）

## DICOM Network Workflows

| Workflow | Services Used | 用途 |
|----------|--------------|---------|
...
**Requirement implication:** requirements 中始终使用 **formal name**；只有需要 cross-reference 时才在 parentheses 中包含 marketing name。

## IntelliSpace Portal (ISP) — Post-Processing Workstation

ISP 是 Philips 的 **client-server multi-user post-processing platform**，用于 advanced visualization。它与 CT scanner console 互补：**console acquires**，ISP **analyzes**。

### Architecture

- **Server** — 运行 computation，central storage patient data
- **Client** — thin client 通过 LAN/WAN/VPN 连接；支持 Citrix XenDesktop（with image-quality caveat）
- **Multi-user** — dual-server config 每套最多 15 concurrent clients
- **Threshold** — ~25k slices（2-5 clients）/ ~30k（6-10 clients）/ ~60k（11-15 clients, dual server）
- **Client/server version must match** — 不可连接 mismatched server
