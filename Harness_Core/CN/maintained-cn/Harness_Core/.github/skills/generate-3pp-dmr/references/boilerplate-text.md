...
`generate-3pp-dmr` skill 的 companion reference。使用这些 strings 时必须 **verbatim**。不要 paraphrase — 它们是 released 3PP DMRs 中的 standard wording。
...
```
SAS interface feature（applicable 时添加）:
```
...
当 inputs 中出现 product-specific terms（例如 SAS = Start Automatic Scan、12NC = 12 Numeric Code、CANOpen）时添加。
