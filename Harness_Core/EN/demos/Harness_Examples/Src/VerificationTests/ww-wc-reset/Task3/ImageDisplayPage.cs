...
internal sealed class ImageDisplayPage
{
    ...
    /// <summary>Gets the current text shown in the status bar notification area.</summary>
    internal string StatusBarText => GetElementText("StatusBarMessage");
    ...
    /// <summary>Waits until the Window Center display reaches <paramref name="expectedCenterInHu"/>, up to <paramref name="timeout"/>.</summary>
    internal void WaitForWindowCenter(int expectedCenterInHu, TimeSpan timeout) =>
        Retry.WhileFalse(
            () => WindowCenter == expectedCenterInHu,
            timeout,
            TimeSpan.FromMilliseconds(100));

    /// <summary>Waits until the status bar text becomes non-empty, up to <paramref name="timeout"/>.</summary>
    internal void WaitForStatusBarNonEmpty(TimeSpan timeout) =>
        Retry.WhileFalse(
