...
internal sealed class ImageDisplayPage
{
    ...
    /// <summary>Waits until the status bar text becomes empty (auto-dismissed), up to <paramref name="timeout"/>.</summary>
    internal void WaitForStatusBarEmpty(TimeSpan timeout) =>
        Retry.WhileFalse(
            () => string.IsNullOrEmpty(StatusBarText),
            timeout,
            TimeSpan.FromMilliseconds(100));
