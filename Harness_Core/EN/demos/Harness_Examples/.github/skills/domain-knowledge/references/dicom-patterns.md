...
| (0020,0032) | ImagePositionPatient | DS/3 | Top-left pixel in patient coordinates (x\y\z) |
| (0020,0037) | ImageOrientationPatient | DS/6 | Row + Column direction cosines |
| (0028,0010) | Rows | US/1 | Pixel matrix height (typically 512) |
| (0028,0011) | Columns | US/1 | Pixel matrix width (typically 512) |
...
| Name | UID | Compression | Notes |
|------|-----|-------------|-------|
| Implicit VR LE | 1.2.840.10008.1.2 | None | Default, all devices must support |
| Explicit VR LE | 1.2.840.10008.1.2.1 | None | Recommended, debug-friendly |
| JPEG Lossless SV1 | ...4.70 | Lossless | Required for CT post-processing |
| JPEG 2000 Lossless | ...4.90 | Lossless | Modern preferred |

**Rule:** CT post-processing must use **lossless** transfer syntax to preserve HU precision.
...
- Odd group numbers are private (e.g., `01F1`, `7005`)
- Elements `(gg,0010)`–`(gg,00FF)` are creator reservation slots
- Each creator owns 256 tags
- Anonymization must delete all private tags (may leak info)
