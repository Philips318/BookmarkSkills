...
在Kotlin中构建MCP服务器时，请使用官方Kotlin SDK遵循这些最佳实践和模式。

##服务器设置

使用`Server`类创建MCP服务器：```kotlin
...
使用`server.addTool()`注册工具，输入request/response处理：```kotlin
...
使用`server.addResource()`提供可访问的数据：```kotlin
...
使用`server.addPrompt()`作为可重用的提示模板：```kotlin
...
所有MCP操作都暂停了。正确使用Kotlin协程：```kotlin
...
使用Kotlin的异常处理并提供有意义的错误消息：```kotlin
server.addTool(
    name = "validate-input",
...
```
使用kotlinx.serialization的JSON模式

使用kotlinx。类型安全JSON模式的序列化：```kotlin
...
Kotlin SDK支持Kotlin多平台（JVM, Wasm, iOS）：```kotlin
kotlin {
    jvm()
    js(IR) {
        browser()
...
使用Kotlin协程测试工具测试您的MCP工具：```kotlin
