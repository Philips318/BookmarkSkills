...
> **可运行示例：** [recipe/RalphLoop.java]（recipe/RalphLoop.java）
>
>“bash
> jbangrecipe/RalphLoop.java> ' ' '
...
最小的Ralph循环——相当于SDK中的`while :; do cat PROMPT.md | copilot ; done`：```java
...
完整的拉尔夫模式与规划和建筑模式，匹配[拉尔夫Playbook]（https://github.com/ClaytonFarr/ralph-playbook）架构：```java
///usr/bin/env jbang "$0" "$@" ; exit $?
//DEPS com.github:copilot-sdk-java:0.2.1-java.1
...
10. **自动审批权限**：使用`PermissionHandler.APPROVE_ALL`允许工具调用而不中断循环何时使用Ralph Loop
