...
最小的Ralph循环——相当于SDK中的`while :; do cat PROMPT.md | copilot ; done`：```go
package main
...
完整的拉尔夫模式与规划和建筑模式，匹配[拉尔夫Playbook]（https://github.com/ClaytonFarr/ralph-playbook）架构：```go
package main
