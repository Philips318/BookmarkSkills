...
这是`quality/REFINE_REQUIREMENTS.md`的模板。剧本与需求管道输出一起生成这个文件。它提供了基于评审反馈更新需求的结构化过程，并且可以与任何模型一起运行。

##生成文件模板

剧本应该生成如下`quality/REFINE_REQUIREMENTS.md`：
...
###所做的更改
- REQ-NNN (NEW):[简要描述]-地址反馈：“[引用提示]”
- REQ-NNN：增加了[what]的满意条件-地址反馈：“[引用提示]”
- REQ-NNN：尖锐的条件#N:[什么改变了]-地址反馈：“[引号提示]”
用例N：在需求列表中增加REQ-NNN

未解决反馈项
原因：[为什么这不是可操作的或者超出了范围]

# # #总结
新增N个需求，修改N个现有需求，更新N个用例。
总需求：N（为N）。```
