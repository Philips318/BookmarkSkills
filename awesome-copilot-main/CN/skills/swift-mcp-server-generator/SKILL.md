...
使用Swift官方SDK包在Swift中生成一个完整的、生产就绪的MCP服务器。

##项目生成

当被要求创建Swift MCP服务器时，用这个结构生成一个完整的项目：```
my-mcp-server/
├── Package.swift
├── Sources/
│   └── MyMCPServer/
...
```
## ToolDefinitions.swift Template```swift
import MCP
...
```
## ResourceDefinitions.swift Template```swift
import MCP
...
```
## PromptDefinitions.swift Template```swift
import MCP
...
添加到`claude_desktop_config.json`：```json
{
  "mcpServers": {
    "my-mcp-server": {
