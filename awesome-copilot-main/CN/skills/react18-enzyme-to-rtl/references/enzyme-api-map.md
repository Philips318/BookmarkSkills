#酶API地图-完成Before/After##设置/配置```jsx
// Enzyme:
...
```
**使用`userEvent`进行大多数交互** -它像真正的用户一样触发完整的事件序列（指针向下，鼠标向下，焦点，点击等）。仅在测试特定事件属性时使用`fireEvent`。

---

关于Props和State的断言```jsx
...
```
**关键原则：**不要测试状态值——测试状态在UI中产生了什么。如果组件呈现`<span>Count: {this.state.count}</span>`，测试该跨度。
