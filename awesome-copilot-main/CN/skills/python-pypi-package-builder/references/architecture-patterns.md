...
##目录
1. [后端系统（Plugin/Strategy模式）]（#1-backend-system-pluginstrategy-pattern）
2. [配置层（设置数据类）]（#2- Config - Layer - Settings -data类）
3. [传输层（HTTP客户端抽象）]（#3-transport-layer-http-client-abstraction）
4. (CLI支持)(# 4-cli-support)
5. [后端注入核心客户端]（#5-backend-injection-in-core-client）
6. (决策规则)(# 6-decision-rules)

---

# # 1。后端系统（Plugin/Strategy模式）

使用明确的基本协议（零依赖默认值）构建`backends/`子包
实现，以及附加功能后面可选的繁重实现。
...
```
###`backends/memory.py`-默认零深度实现```python
...
```
###`backends/redis.py`-可选的重型实现```python
...
class RedisBackend(BaseBackend):
    ...
    def close(self) -> None:
        self._client.close()
```
###`backends/__init__.py`-公共API +工厂```python
...
将所有配置集中在一个`config.py`模块中。避免分散魔法值和
跨代码库的`os.environ`调用。### `config.py`
...
隔离所有HTTP关注-标头，重试，超时，错误解析-在一个专用的`transport/`包里。核心客户端依赖于传输抽象，而不是`httpx`或者直接写成`requests`。
...
通过`pyproject.toml`中的`[project.scripts]`添加CLI入口点。`pyproject.toml`入口```toml
[project.scripts]
your-cli = "your_package.cli:main"
```
安装完成后，用户可以直接从终端运行`your-cli --help`。
...
**临界：**总是接受`backend`作为构造函数参数。永远不要实例化后端
在没有回退参数的构造函数内部—这使得测试无法进行。```python
