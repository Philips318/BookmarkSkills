#骨架：0-assessment.md> **⛔复制下面的模板内容（不包括外部代码栏）。替换`[FILL]`占位符。不要add/rename/reorder部分
>`[FILL]`=单个值|`[FILL-PROSE]`=段落|`[REPEAT]...[END-REPEAT]`= N个副本|`[CONDITIONAL]...[END-CONDITIONAL]`=包含如果条件满足
...
```
**这个框架中包含的关键格式规则
-`0-assessment.md`是报告文件中的第一行（而不是`0.1-architecture.md`）
-`## Analysis Context & Assumptions`使用`&`（没有单词“and”）
-每对`## `段之间的`---`水平规则（最少6条）
-`### Quick Wins`始终存在（如果没有低努力的发现，有回退说明）
-`### Needs Verification`和`### Finding Overrides`始终存在（即使`—`为空）
-引用有两个三列表的子部分（而不是扁平的两列表）
-所有元数据值都用反引号括起来
-所有元数据字段（模型、已开始分析、已完成分析、持续时间）
-风险评级标题没有表情符号
-行动总结有4个数据行：第1层，第2层，第3层，总数-没有“缓解”或“平台”行
- Git Commit行包括提交日期在括号中：`SHA`（`date`）