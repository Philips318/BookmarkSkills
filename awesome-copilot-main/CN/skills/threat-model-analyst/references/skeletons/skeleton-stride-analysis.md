#骨架：2-stride-analysis.md> **⛔复制下面的模板内容（不包括外部代码栏）。替换`[FILL]`占位符。STRIDE-A中的“A”永远是“滥用”，而不是“授权”
b> **⛔可利用层表必须有4列：`Tier | Label | Prerequisites | Assignment Rule`。不要合并成3列。不要将`Assignment Rule`重命名为`Description`.**
b> **⛔汇总表必须包含一个`Link`列：`Component | Link | S | T | R | I | D | E | A | Total | T1 | T2 | T3 | Risk`**
> **⛔N/A类别必须使用一个表（`| Category | Justification |`），而不是prose/bullet点
...
```
**跨步+滥用案件- 7类确切：**
欺骗|篡改|拒绝|信息泄露|拒绝服务|特权提升|滥用

**注：**前6步是标准跨步。“滥用”是业务逻辑滥用（工作流操作、特性开发、API滥用）的补充类别。它不是“授权”——授权问题属于特权提升(E)。

**有效状态值：**`Open`|`Mitigated`|`Platform`-不允许其他值。