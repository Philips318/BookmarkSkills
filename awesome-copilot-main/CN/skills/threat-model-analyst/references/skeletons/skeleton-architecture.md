#骨架：0.1-architecture.md> **⛔复制下面的模板内容（不包括外部代码栏）。替换`[FILL]`占位符。不要add/rename/reorder部分
> **⛔关键组件表的列是EXACTLY:`Component | Type | Description`。不要重命名为`Role`，`Change`,`Function`.**
> **⛔技术栈表列是EXACTLY:`Layer | Technologies`（2列）。不添加`Version`列或将`Layer`重命名为`Category`.**
> **⛔安全基础设施清单和存储库结构部分是强制性的-不要省略它们
...
```mermaid
[FILL：使用service/external/datastore样式的架构图-不是DFD圆圈]```
...
```mermaid
sequenceDiagram    [FILL: participants, messages, alt/opt blocks]
```
