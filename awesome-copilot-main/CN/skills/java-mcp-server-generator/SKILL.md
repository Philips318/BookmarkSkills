...
使用官方Java SDK与Maven或Gradle在Java中生成一个完整的、生产就绪的MCP服务器。

##项目生成

当被要求创建一个Java MCP服务器时，用这个结构生成一个完整的项目：```
my-mcp-server/
...
```
Gradle build.gradle.kts模板```kotlin
plugins {
    id("java")
    id("application")
}
...
```
##McpServerApplication.java模板```java
...
```
##ResourceDefinitions.java模板```java
...
### Maven
```bash
Java -jartarget/my-mcp-server-1.0.0.jar```
