...
使用官方的Ruby SDK用Ruby生成一个完整的、生产就绪的MCP服务器。

##项目生成

当被要求创建一个Ruby MCP服务器时，用这个结构生成一个完整的项目：```
my-mcp-server/
├── Gemfile
├── Rakefile
├── lib/
...
```
##lib/my_mcp_server/server.rb模板```ruby
# frozen_string_literal: true
...
```
##lib/my_mcp_server/tools/greet_tool.rb模板```ruby
# frozen_string_literal: true
...
```
##lib/my_mcp_server/tools/calculate_tool.rb模板```ruby
# frozen_string_literal: true
...
```
##lib/my_mcp_server/prompts/code_review_prompt.rb模板```ruby
# frozen_string_literal: true
...
```
##lib/my_mcp_server/resources/example_resource.rb模板```ruby
# frozen_string_literal: true
...
```
##test/tools/greet_tool_test.rb模板```ruby
# frozen_string_literal: true
...
```
##test/tools/calculate_tool_test.rb模板```ruby
# frozen_string_literal: true
...
```bash
Bundle执行bin/mcp-server```

Then send JSON-RPC requests:

```bash
{“jsonrpc”:“2.0”,“id”:“1”,“方法”:“萍”}
{“jsonrpc”:“2.0”,“id”:“2”,“方法”:“tools/list”}
{“jsonrpc”:“2.0”,“id”:“3”,“方法”:“tools/call”,“参数”:{“名称”:“打招呼”,“参数”:{“名称”:“Ruby”}}}```
...
```ruby
类McpController < ApplicationController
def指数    server = MyMcpServer::Server.new(
...
1. **询问项目名称和描述**
2. **生成具有适当命名和模块结构的所有文件**
3. **使用类的工具和提示，以更好地组织
4. **包括input/output模式**用于类型安全
5. **为行为提示添加工具注释**
6. **在回复中包含结构化内容**
7. **对所有工具进行全面测试**
8. **遵循Ruby约定** （snake_case, modules, frozen_string_literal）
9. **添加正确的错误处理**与is_error标志
10. **提供两个工作室和HTTP**使用的例子