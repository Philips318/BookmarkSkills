...
将此模板复制到您的项目根目录并填写每个部分。**不要缩写12-14节** -它们对交叉聊天上下文的生存至关重要。
...
npm安装
CD API && NPM安装
Cpapi/local.settings.json.exampleapi/local.settings.jsonNPM运行dev:all```
...
**Setup:**
```bash
Git克隆<repo><folder-name>cd<folder-name>Git checkout -b<branch-name>npm安装```
